...
### November 2024
- **Version 11.0 Added (May 2020)**: MAJOR version with 31 features including river reach 'build all' feature, clip meshing prototype (performance improvements for complex geometries), mixed resolution gridded ground model imports (<1m tiles), snapping mode vertex highlighting, user-defined engineering validation (SQL-like syntax), base time for runs, HYDX network import, Hong Kong 5th edition design rainfall, ArcGIS 10.8 & MapXtreme 9.2.0 support, enhanced rainfall CSV import, comprehensive SWMM enhancements (design rainfall, sub-events, low depth Manning's N, infiltration modeling per subcatchment, additional DWF inflows, initial state simulations, runoff results, summary/water quality results, InfoSWMM import, network/results export to GIS, SWMM5 encoding systems, name generation), and spatial TSDB performance improvements
- **Version 2021.1 Added (November 2020)**: MAJOR version with 29 features including version numbering change to 2021.x system, clip meshing as default (4x faster than classic), Thales licence support, workgroup database groups, excluded object selection/restoration tools, extending cross sections with ground model integration, straightening lines, Manning's roughness weighting choice (HEC-RAS compatibility), TIN ground model import (12D/XPTIN/LandXML), subcatchment drainage to 2D point source, >10x faster 2D flood mapping, minimum flood depth display, 2D zone 3D rendering options, rectangular flap valve type, day/night coliform decay, updated diffusion defaults, comprehensive XPRAFTS/XPSWMM/XPStorm data import capabilities, and multiple modeling/water quality enhancements
- **Version 2021.2 Added (February 2021)**: Comprehensive version with 13 features including FEH2013 custom timesteps, Canadian GRIB2 radar data support, GeoTIFF ground model import, InfoDrainage IDDX import, multi-coliforms modelling (CF1-CF4 with sediment attachment), XPRAFTS retarding basin import, statistics template network type selection, SWMM mass unit factor for pollutant inflows, rain gage boundaries for SWMM, SWMM5 export updates, SWMM engine v5.1.015, General Lines for SWMM, and Flood type property for SWMM nodes
- **Version 2021.3 Added (March 2021)**: Major version with 9 features including multiple timestep intervals for FEH2013 rainfall generator, minute granularity for spatial TSDB files, HYDX import improvements, comprehensive 2D modelling for SWMM networks (2D Zones, Mesh Zones, Porous polygons, meshing tools, results display), Total head results for 1D network points, 2D permeable zone area results, Green-Ampt SMD reset for 2D simulations, area-averaged subcatchment rainfall for SWMM, and SWMM engine update to v5.1.015
- **Version 2021.4 Added (May 2021)**: Comprehensive version with 10 features including Roughness definitions for InfoWorks networks (depth-dependent Manning's n), Finite Volume solution model for trans-critical flow in conduits (prototype), Malaysia HP1 2015 rainfall design, HYDX importer improvements, XPSWMM/XPStorm data import to InfoWorks and SWMM networks, SWMM 2D node flooding discharge coefficient, grid window sum/average display, and Roughness/Mesh Level Zones for SWMM networks
- **Version 2021.5 Added (June 2021)**: Focused version with 10 features including 12D TIN ground model import enhancements, recycle bin performance improvements, SWMM conduit default elevation flags, spatially varying rainfall for SWMM 2D mesh elements, Porous Walls for SWMM networks, extended XPSWMM/XPStorm import to InfoWorks (subcatchments, runoff surfaces, 2D objects), 3D zone rendering fixes, and SWMM water quality results display enhancements with new pollutant units (PCC, PCU)
- **Version 2021.6 Added (August 2021)**: Major version with 18 features including Results selectors for InfoWorks networks, 90% faster 2D preprocessing, culvert inlet/outlet behavior improvements, Chicago and Malaysia HP1 rainfall generators, GPU support for InfoWorks and SWMM 2D simulations, SWMM 2D Zone boundary types, SWMM outfall node enhancements, dynamic roughness definitions for SWMM, water quality statistics templates, improved XPSWMM/XPStorm link import, XPRAFTS subcatchment import options, and desktop analytics
- **Version 2021.7 Added (September 2021)**: Compact version with 4 features including NOAA ATLAS 14 Rainfall Generator for US precipitation frequency estimates, Thales licence expiry warnings, InfoWorks About box redesign with Additional Information window, and SCS runoff data import from XPSWMM/XPStorm to InfoWorks networks
...
- **Timeline Coverage**: Application displays ~30 version entries from 2026.2 back to 1.5 (February 2011). Currently 33 versions have full content and 0 placeholders. **Complete continuous coverage** from Version 1.5 (Feb 2011) through Version 2023.2 (Oct 2022), with partial 2024-2026 coverage. **Notable gap**: ~2.5-year period between Version 8.5 (Nov 2017) and Version 11.0 (May 2020) remains for intermediate versions (9.0, 9.5, 10.0, 10.5) - these versions may not exist or may be documented under different numbering.
