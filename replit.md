...
### November 2024
- **Version 2023.2 Added (October 2022)**: Comprehensive version with 17 features including overlapping roughness zones with priority-based meshing, ground model theme enhancements, CNSWMM runoff volume model, SCS runoff models (curvilinear/triangular) for SWMM subcatchments, comprehensive XPSWMM/XPStorm XPX import enhancements (SCS hydrology, roughness zones, Manning's roughness), SWMM5 infiltration export/import improvements, alert graph trace colors, RTC regulator precision increase, and section editor ground model updates
- **Version 8.5 Added (November 2017)**: Comprehensive version with 24 features including massive rainfall profile capacity (999M+ profiles), TSDB enhancements, HEC-RAS 2D geometry extensions, SWMM v5.1.012 engine update, time-varying blockages, permeable zones for infiltrated volume transfer, multi-barrel links, enhanced 2D linear structure headloss, SWMM5 RDII options, Australian Rainfall (2016) generator, and comprehensive 2D modeling improvements
...
- **Timeline Coverage**: Application displays ~30 version entries from 2026.2 back to 1.5 (February 2011). Currently 22 versions have full content and 8 are placeholders. All populated versions have comprehensive feature documentation with no partial content remaining. User can provide "What's New" documentation to populate remaining versions. **Notable gap**: 5-year period between Version 8.5 (Nov 2017) and Version 2023.2 (Oct 2022) with missing intermediate versions (9.0, 9.5, 10.0, 10.5, 11.0).
